...
for ii=1:1:length(snrdb)
    % to geneterate number of recieved particles for estimated data on-off
    % keying
    for i = 2:L
    ...
    if(data(1)==0) && (data_est(1)==1)
            temp=last_term;
        elseif(data(1)==0) && (data_est(1)==0)
            temp=1-last_term;
        elseif(data(1)==1) && (data_est(1)==0)
            temp=first_term;
        else
            temp=1-first_term;
    end
     % for second value of chanel length
    for jj=1:1:length(x) 
     ...
     %for rest value of chanel length (because it is 2 bit reciever so need of two previous values)
    for kk=3:1:L
        %sie function logic
        ...
        if(data(kk-2)==0) && (data_est(kk-2)==1)
            temp2=last_term;
        elseif(data(kk-2)==0) && (data_est(kk-2)==0)
            temp2=1-last_term;
        elseif(data(kk-2)==1) && (data_est(kk-2)==0)
            temp2=first_term;
        elseif(data(kk-2)==1) && (data_est(kk-2)==1)
            temp2=1-first_term;
        end
                
        for jj=1:1:length(x) 
      
            first_term = n + (1-Q_fun(lam2, x(jj)))*temp1*temp2;
            last_term = m + Q_fun(lam1, x(jj))*temp1*temp2;
            final_term_next = 0.5*(first_term + last_term);
            if(final_term_next < final_term)
                final_term = final_term_next;
                final_tao = x(jj);
...
ylabel('BER')
legend( 'Equiprobability two bit mem. Rx. BER','Optimal two bit mem. Rx. BER','Location','southwest');
savefig('fig16.fig');
